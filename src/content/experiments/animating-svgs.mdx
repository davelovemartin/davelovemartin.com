...
SVG contains a few elements that control animations: `<animate />`, `<animateMotion />` and `<animateTransform />`.

Adding an animate element as a child to a shape, allows you to animate properties of its parent.
